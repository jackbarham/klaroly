...
export const colourGroups: ColourGroup[] = [
  {
    ...
    tokens: [
      ...
      { token: '--surface-hover', className: 'bg-surface-hover', use: "the second step of hover, for something already sunken: today's cell on the calendar, and the month title under the pointer" },
  ...
  },
  {
    ...
    tokens: [
      ...
      { token: '--border-accent-soft', className: 'bg-border-accent-soft', use: 'a radio card under the pointer, and a provisional booking\'s spine in the list: the accent softened by the ground behind it' },
    ],
  },
  {
    title: 'Accent',
    ...
    tokens: [
      ...
      { token: '--accent-subtle', className: 'bg-accent-subtle', use: "a tinted accent background: the calendar's quiet actions on hover, and the chosen year in the month jump sheet" },
    ],
  },
  {
    title: 'Status',
    ...
    tokens: [
      ...
      { token: '--danger-subtle', className: 'bg-danger-subtle', use: 'an error or cancelled pill: a lost enquiry and a cancelled booking in the bookings list' },
      ...
      { token: '--warning', className: 'bg-warning', use: "awaiting, due: an enquiry's spine in the bookings list" },
      { token: '--warning-solid', className: 'bg-warning-solid', use: 'a warning fill carrying a white label, which is the enquiry count badge on a calendar day. White on --warning is 4.1:1 and fails; on this it is 6.2:1' },
  ...
  {
    title: 'Calendar',
    note: 'Two tokens the bookings screen needed and nothing else in the app had. --row-hover is half the strength of --surface-sunken in both themes, because at full strength a hovered row reads as one of the sticky group headings above it. --cal-line matches --border in light and goes a step fainter in dark, where seven columns by six rows of white 12% reads as a table rather than as a diary.',
    tokens: [
      { token: '--cal-line', className: 'bg-cal-line', use: 'the rule between days on the month grid, drawn as 1px gaps over this colour rather than as a border on each cell' },
      { token: '--row-hover', className: 'bg-row-hover', use: 'a booking row under the pointer. Half the strength of the sunken grey, in both themes' },
    ],
  },
]
