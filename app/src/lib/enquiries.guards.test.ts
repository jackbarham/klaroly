...
describe('the enquiries feature', () => {
  ...
  it('never makes the list a listbox', () => {
    expect(offences(/role="(listbox|option)"/, files)).toEqual([])
    expect(offences(/aria-activedescendant/, files)).toEqual([])
    expect(offences(/role="combobox"/, files)).toEqual([])
  })

  // Paired with the rule above, so it cannot pass on a feature that renders no
  // roles at all.
  it('does make it a list', () => {
    expect(offences(/role="list"/, files).length).toBeGreaterThan(0)
  })
  ...
  it('never computes a cold threshold of its own', () => {
    expect(offences(/cold_enquiry_days|coldDays|COLD_DAYS/, files)).toEqual([])
  })
})
